...
| 스크립트 | 책임 |
|---|---|
| `SkillCatalogLogic` | 데이터셋 파싱·lazy 캐시 (BoardCatalogLogic 미러) + grid→오프셋 변환 캐시 + `jobTierIndex[job][tier]` 풀 인덱스. 게터 이원화: `GetSkill`(deep-copy — 변이 가능 소비자용, 예: 인스턴스 생성) / `GetSkillRO`(원본 참조, **변이 금지 계약** — 필터·표기 등 읽기 핫패스용). TagDef/RangePattern도 동일 쌍 |
| `SkillInstanceLogic` | SkillInstance 생성/레벨업/합성(전부 승계)/교체(1계승)/재추첨 — 유일한 def 변이·인스턴스 조작 지점(deep-copy 게터 사용) |
| `ChoiceRollerLogic` | **무상태** 레벨업 선택지 롤러 (2026-07-10 Growth에서 분리). 전직 카드/합성/능력치/스킬 후보 조립 + `FilterSkillCandidates`(자격)·`BuildSkillChoice`(표기) 공용 헬퍼. **새 선택지 종류(장비 등)는 여기 한 곳만 확장** |
| `CombatResolveLogic` | **무상태** 공격 해석기. `(run, def)`를 인자로 받아 BoardService 질의로 타겟 확정 → HitPlan 산출 (§5). 연타(castAdd)는 여기가 아니라 젠 플로우의 cast 단위 큐가 발동 반복 |
| `CombatFxLogic` | 네이티브 데미지 스킨 재생 (Multicast 중계) |
| `GameStartLogic` | 로비 시작 요청 → 보드 생성 → 런 컴포넌트 부착·개시 |

UI 컨트롤러 (`Character/UI/`, 2026-07-10 3분할): `InGameHUDController`(페이즈/프로필/방향/메인 버튼) · `InGameChoiceController`(선택지+만석 교체 오버레이) · `InGameSlotController`(발동 순서/스왑/툴팁/재추첨/발동 강조) · `LobbyUIController`(직업 선택 → 시작)
...
PlayerGrowthComponent   AddExp → 레벨업 → RollChoicesNow(ChoiceRollerLogic 위임) → CommitChoice 분기
                        (skill/synthesis/stat/skillTag/advance — 전직 카드는 커밋 시 연출+선택지 즉시
                        재추첨하며 새 차수 스킬 1장 보장) + 만석 교체 대기(pendingReplace) 상태머신

PlayerJobComponent      job, tier(1~4), CanAdvance(선택지 3회 노출+전리품) / Advance(전리품 소모, 풀 해금)
                        / SpendLoot(재추첨 등 공용 소비 입구)
```
...
- ⚠ 직렬화 시 알려진 변환 지점 2곳: `SkillLoadoutComponent.order`는 **인스턴스 참조 배열**이라 attackSlots 인덱스 배열로 변환해 저장(복원은 기존 `SetOrder(indices)` 재사용) / `PlayerStatComponent.modifiers.turnsLeft`는 파싱 후 값이라 원본 `duration` 문자열 대신 turnsLeft 숫자를 그대로 저장. 노출 중 `pendingChoices/pendingReplace`는 저장하지 않고 복원 시 재추첨.
