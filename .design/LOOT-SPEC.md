...
## 5. UI: 인게임 인벤토리 (`ui/LootInventory.ui`) — 기획서 v0.2 구조 (2026-07-12 전면 개정)

> **SSOT = 드라이브 "인게임 인벤토리 UI 기획서"(v0.2 확정).** 초기의 "옛메 24슬롯 그리드 + 메소 줄" 디자인은
> 기획서 확인(2026-07-12)으로 **폐기** — 기획서가 확정한 토글 오버레이 구조로 재작업됨.

- **진입:** 좌하단 인벤 버튼(우하단은 젠↔공격 메인 CTA 자리 — 인게임 화면설계서 ⑫). 열람은 전투 정지 없음.
- **패널:** 중앙 다크 오버레이(`#141A28` .95) + 반투명 백드롭(바깥 탭 닫기) + 우상단 X.
- **토글:** 최상단 [장비]·[전리품] 2탭, 마지막 본 탭 유지. 탭별 스코프 캡션 상시 노출.
- **장비 탭(1차 = 빈 상태):** "아직 획득한 장비가 없습니다" — 장비 시스템(레벨업 선택지 10% 드랍) 연동 시
  가로 스크롤 아이콘 그리드 + New 뱃지 + 탭 팝업(이름/효과/플레이버/획득 젠)으로 확장(기획서 3-2).
- **전리품 탭:**
  - **전직 재료 고정 카드** = `검은 구슬`(jobMaterial=true, 2026-07-12 확정) — "런 한정" 배지 + 보유 수량.
    보유/필요 진행률(7/12식)은 전직 시스템의 필요 수량 확정 시 추가.
  - **등급별 소재**: 브론즈/실버/골드 3탭(개수 뱃지) → 소재 목록(아이콘·이름·보유수, 보유 내림차순, 5행).
    빈 등급은 "아직 획득한 소재가 없습니다". 획득 확률 수치 비노출(컴플라이언스 §7 선행).
  - 메소 줄은 **임시 표기** — 재화 표시는 재화 시스템 설계서 소관으로 이관 예정.
- **인터랙션:** 탭 1차(모바일 우선). 버튼 클릭/호버 SFX 유지.
- **경계:** 이 창은 `.ui` 캔버스 소관 — 보드 SortingLayer 체계(BOARD-SPEC §1.5) 밖.

### 5.5 기획서 정합 게이트 (후속 — 2026-07-12 등록)

1. **계정 귀속·누적 소재**: 기획서는 등급별 소재를 계정 귀속(런 종료에도 유지)으로 확정 — 현 구현은 런 휘발.
   DataStorage 저장 파이프(§8 보상 정산과 함께) 설계 필요. UI 배지는 기획서 표기를 선반영.
2. **장비 탭 실데이터**: 장비 시스템(레벨업 선택지) 구현 시 3-2 스펙(그리드·New 뱃지·팝업·상한 30안) 배선.
3. **전직 재료 필요 수량**: `requiredForJobChange` — 전직 시스템 확정 시 진행률 표기.
4. LootTypes 확장 완료분: `grade`(bronze/silver/gold, 일반 소재 bronze 폴백) + `jobMaterial`(bool) 컬럼(2026-07-12).
